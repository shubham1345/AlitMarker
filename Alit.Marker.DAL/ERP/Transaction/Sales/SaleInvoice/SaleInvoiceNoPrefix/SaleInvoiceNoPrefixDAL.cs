...
namespace Alit.Marker.DAL.ERP.Transaction.Sales.SaleInvoice.SaleInvoiceNoPrefix
{
    public class SaleInvoiceNoPrefixDAL : IGridCRUDDAL, ICRUDDAL, ILookupListDAL
    {
        public SavingResult SaveRecord(IGridCRUDViewModel ViewModel)
        {
            return SaveRecord((SaleInvoiceNoPrefixViewModel)ViewModel);
        }
        ...
        public SavingResult SaveRecord(SaleInvoiceNoPrefixViewModel ViewModel)
        {
            ...
            {
                if (IsDuplicateRecord(ViewModel.PrefixName, ViewModel.SaleInvoiceNoPrefixID, db))
                {
                ...
                tblSaleInvoiceNoPrefix SaveModel = null;
                if (ViewModel.SaleInvoiceNoPrefixID == 0) // New Entry
                {
                    SaveModel = new tblSaleInvoiceNoPrefix();
                    SaveModel.rcuid = Model.CommonProperties.LoginInfo.LoggedinUser.UserID;
                    SaveModel.rcdt = DateTime.Now;
                    SaveModel.CompanyID = Model.CommonProperties.LoginInfo.LoggedInCompany.CompanyID;
                    db.tblSaleInvoiceNoPrefixes.Add(SaveModel);
                }
                else
                {
                    SaveModel = db.tblSaleInvoiceNoPrefixes.Find(ViewModel.SaleInvoiceNoPrefixID);
                    ...
                    db.tblSaleInvoiceNoPrefixes.Attach(SaveModel);
                ...
                {
                    db.SaveChanges();
                    res.PrimeKeyValue = SaveModel.SaleInvoiceNoPrefixID;
        ...
        public BeforeDeleteValidationResult ValidateBeforeDelete(long DeleteID)
        {
            ...
            using (dbMarkerEntities db = new dbMarkerEntities())
            {
                if (db.tblSaleInvoices.Any(r => r.CompanyID == Model.CommonProperties.LoginInfo.LoggedInCompany.CompanyID && r.SaleInvoiceNoPrefixID == DeleteID))
                {
                    Result.ValidationMessage = "Prefix exists in some Sale Invoice.";
                }
            }
        ...
        public SavingResult DeleteRecord(long DeleteID)
        {
            ...
            using (dbMarkerEntities db = new dbMarkerEntities())
            {
                if (DeleteID != 0)
                {
                    tblSaleInvoiceNoPrefix RecordToDelete = db.tblSaleInvoiceNoPrefixes.FirstOrDefault(r => r.SaleInvoiceNoPrefixID == DeleteID);
        ...
        public SavingResult DeleteRecord(tblSaleInvoiceNoPrefix RecordToDelete, dbMarkerEntities db)
        {
            SavingResult res = new SavingResult();
            ...
            {
                db.tblSaleInvoiceNoPrefixes.Remove(RecordToDelete);
            }
            return res;
        }
        ...
        public IEnumerable<IGridCRUDViewModel> GetViewModelList(object[] FilterParas = null)
        {
            using (dbMarkerEntities db = new dbMarkerEntities())
            {
                return (from r in db.tblSaleInvoiceNoPrefixes
                        ...
                        select new SaleInvoiceNoPrefixViewModel()
                        {
        ...
        public SaleInvoiceNoPrefixViewModel GetViewModelByPrimeKey(long ID)
        {
            using (dbMarkerEntities db = new dbMarkerEntities())
            {
                return (from r in db.tblSaleInvoiceNoPrefixes
                        ...
                        where r.SaleInvoiceNoPrefixID == ID
        ...
        public SavingResult UpdateRecordState(long ID, eRecordState newRecordState)
        {
            ...
            using (dbMarkerEntities db = new dbMarkerEntities())
            {
                var SaveModel = db.tblSaleInvoiceNoPrefixes.Find(ID);
                if (SaveModel == null)
                {
                    res.ExecutionResult = eExecutionResult.ValidationError;
        ...
        public bool IsDuplicateRecord(string PrefixName, long ID, dbMarkerEntities db)
        {
            ...
            return db.tblSaleInvoiceNoPrefixes.Any(i => i.CompanyID == Model.CommonProperties.LoginInfo.LoggedInCompany.CompanyID
                                                        && i.PrefixName.ToUpper() == PrefixName
                                                        && i.SaleInvoiceNoPrefixID != ID);
        }               
        ...
        public List<SaleInvoiceNoPrefixLookupListModel> GetLookupList()
        {
            using (dbMarkerEntities db = new dbMarkerEntities())
            {
                return (from r in db.tblSaleInvoiceNoPrefixes
                        ...
                        select new SaleInvoiceNoPrefixLookupListModel()
                        {
                            RecordState = (eRecordState)r.rstate,
                            SaleInvoiceNoPrefixID = r.SaleInvoiceNoPrefixID,
                            PrefixName = r.PrefixName,
